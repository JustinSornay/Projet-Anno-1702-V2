...
uses
  Classes, SysUtils, GestionEcran, GestionErreur, ressources, initialisationPrincip,SystemeTour ;
  ...
  //Creation d'une caserne
  PROCEDURE CreateBatCaserne();
...
//                //
  ...
  //Renvoie le Cout du Bois d'une Caserne
  FUNCTION GetCoutBoisCaserne():INTEGER;

  //Renvoie le Cout des Outils d'une Caserne
  FUNCTION GetCoutOutilsCaserne():INTEGER;

  //Renvoie le Cout des pieces d'Or d'une Caserne
  FUNCTION GetCoutOrCaserne():INTEGER;
...
BEGIN
  ...
  CurseurRess.y := CurseurRess.y + 1;
  deplacerCurseur(CurseurRess);
    writeln('Combienn de terrain millitaire voulez-vous construire ?');
    CurseurRess.y := CurseurRess.y + 1;
    deplacerCurseur(CurseurRess);
    AfficheTour();
  deplacerCurseurXY(68, 23);
    Readln(nbBat);
    nbBois := nbBat * CoutBoisCaserne;  //Calcule du nombre de Bois qu'il va falloir utiliser
    nbOr := nbBat * CoutOrCaserne; //Calcule du nombre de piece d'or qu'il va falloir utiliser
    nbOutils := nbBat * CoutOutilsCaserne; //Calcule du nombre d'Outils' qu'il va falloir utiliser

    IF (ComparRess(getGold(),nbOr)) and (ComparRess(getBois(),nbBois)) and (ComparRess(getOutils(),nbOutils)) THEN BEGIN     //Si le joueur a assez de ressources  //Enlevé  OR et nbOutils
      setBois(getBois() - nbBois);  //Retrait du cout des batiments sur le bois du joueur
      setGold(getGold() - nbOr);  //Retrait du cout des batiments sur l'or du joueur
      setOutils(getOutils() - nbOutils);  //Retrait du cout des batiments sur les outils du joueur
      setCaserne(getCaserne() + nbBat); //Ajout du nombre de CentreVille

      CurseurRess.y := CurseurRess.y + 1;
      deplacerCurseur(CurseurRess);

      writeln('Vous avez construit : ',nbBat,' terrain millitaire !');
      CurseurRess.y := CurseurRess.y + 1;
      deplacerCurseur(CurseurRess);
      WriteLn('(',nbBois,' Bois, ',nbOr,' Or, ',nbOutils,' Outils)');
      CurseurRess.y := CurseurRess.y + 1;
...
//Renvoie le Cout du Bois d'une Caserne
  FUNCTION GetCoutBoisCaserne():INTEGER;
  BEGIN
    GetCoutBoisCaserne := CoutBoisCaserne;
  END;

  //Renvoie le Cout des Outils d'une Caserne
  FUNCTION GetCoutOutilsCaserne():INTEGER;
  BEGIN
    GetCoutOutilsCaserne := CoutOutilsCaserne;
  END;

  //Renvoie le Cout des pieces d'Or d'une Caserne
  FUNCTION GetCoutOrCaserne():INTEGER;
  BEGIN
    GetCoutOrCaserne := CoutOrCaserne;
  END;
end.
