...
//Cette fonction initialise les variables de ressource (au début de la partie par exemple)
...
BEGIN
  ...
  writeln('Quantité de Bois         :         ',getBois());   // Affiche la quantitées de Bois
  CurseurRess.y := CurseurRess.y + 1;
  deplacerCurseur(CurseurRess);
  writeln('Quantité de Poissons     :         ',getPoissons());   // Affiche la quantitées de poissons
  CurseurRess.y := CurseurRess.y + 1;
  deplacerCurseur(CurseurRess);
  writeln('Quantité d''Outils        :         ',getOutils());   // Affiche la quantitées d'Outils
  CurseurRess.y := CurseurRess.y + 1;
  deplacerCurseur(CurseurRess);
  writeln('Quantité de laines       :         ',getLaines());   // Affiche la quantitées de laines
  CurseurRess.y := CurseurRess.y + 1;
  deplacerCurseur(CurseurRess);
  writeln('Quantité de tissus       :         ',getTissus());   // Affiche la quantitées de Tissus
END;
