...
//Fonction qui renvoie la valeur de RetourChoixRessources
...
begin
    ...
    InfoRessource();   //Affiche les ressources possédés
...
BEGIN
  ...
  writeln('Poissons     :         ', 'Acheter une unité --> ', MarchandCoutpoissons(), '  Or', '    |   ', 'Vendre une unité --> ',MarchandCoutpoissons());   // Affiche le cout du Poissons
  CurseurRess.y := CurseurRess.y + 1;
  deplacerCurseur(CurseurRess);
  writeln('Outils       :         ', 'Acheter une unité --> ', MarchandCoutOutils(), ' Or', '    |   ', 'Vendre une unité --> ',MarchandCoutOutils());   // Affiche le cout des Outils
  CurseurRess.y := CurseurRess.y + 1;
  deplacerCurseur(CurseurRess);
  writeln('laines       :         ', 'Acheter une unité --> ', MarchandCoutLaine(), '  Or', '    |   ', 'Vendre une unité --> ',MarchandCoutLaine());   // Affiche le cout de la laines
  CurseurRess.y := CurseurRess.y + 1;
  deplacerCurseur(CurseurRess);
  writeln('tissus       :         ', 'Acheter une unité --> ', MarchandCoutTissus(), '  Or', '    |   ', 'Vendre une unité --> ',MarchandCoutTissus());   // Affiche le cout du Tissus

END;

//Procedure qui class les ressource lors de l'achat
Procedure AcheterRessources();
var
  CurseurRess: coordonnees;
...
Begin
   ...
   CASE ChoixRessources OF
     ...
     Bois :
     BEGIN
       //PROCEDURE POUR ACHETER du bois
       AchatBois();
           END;
     ...
     Outils :
     BEGIN
       //PROCEDURE POUR ACHETER des Outils
       AchatOutils();
     END;
  ...
  //Procedure qui class les ressource lors de la vente
  Procedure VendreRessources();
  var
    CurseurRess: coordonnees;
    TakeRessource: integer;
  ...
  Begin
     ...
     CASE ChoixRessources OF
       ...
       Outils :
       BEGIN
         //PROCEDURE POUR Vendre des Outils
         VendreOutils();
       END;

       Laines :
       BEGIN
         //PROCEDURE POUR Vendre de la Laines
         VendreLaines();
       END;
