...
//Cette procedure affiche l'écran de gestion d'armée
Procedure GestionArmee();
...
VAR
 ...
 Procedure GestionDialogue();
  ...
  BEGIN
   ...
   writeln('Gestion armée : 3         Passer un tour : 4 ');
   deplacerCurseurXY(70, 38);
   readln(x);
    ...
    Case x OF
      ...
      3: {Si le joueur veux former des troupe}
      BEGIN
        IF (GetCaserne > 0) THEN
          begin
            IF ((getCaserneSoldat > 0) or (getCaserneOfficier >0 ) or (getCaserneCapitaine >0 )) 
            THEN
              begin
        ...
        ELSE 
          BEGIN
            deplacerCurseurXY(20, 39);
            writeln('Vous devez posséder un terrain millitaire');
            readln();
          END;
      end;
      
      4: {Si le joueur veux passé au tour suivant}
      BEGIN
        TourSuivant();
      END;
...
var CurseurRess:coordonnees;
begin
dessinerCadreXY(0,0,148,49, simple, 15, 0); // Cadre principal de la page
InfoRessource();
affichePersonnage(); //affiche le nom du personnage ainsi que les golds
InfoBatArmee(); // procedure pour afficher les batiments militaires
//InfoCoutBatArmee();
end;
